    ...
    def __init__(self, net_mode='Capsule', routing_type='k_means', num_iterations=3):
        ...
        if self.net_mode == 'Capsule':
            self.classifier = nn.Sequential(
                CapsuleLinear(out_capsules=60, in_length=128, out_length=16, routing_type=routing_type,
                              num_iterations=num_iterations),
                CapsuleLinear(out_capsules=100, in_length=16, out_length=22, routing_type=routing_type,
                              num_iterations=num_iterations))
        else:
